...
async function fillTutorials() {
  ...
  .then(res  => {
    totalPages = Number(res.headers.get("x-wp-total"));
    showPaging();
    return res.json()
  })
  .then(json => {
...
function showPaging(){
  if (page > 1){
    ...
    <a href="tutorials.html?page=${page-1}" ><img src="/images/arrow-left.png" class="arrow-left"></a>
    `
  }

  if (page < totalPages){
    tutorialsPaging.innerHTML += `
    <a href="tutorials.html?page=${page+1}" ><img src="/images/arrow-right.png" class="arrow-right"></a>
    `
